...
interface BreadcrumbEntry {
  id?: string;
  name: string;
  driveId?: string; // presente quando estamos dentro de um drive partilhado
}
...
const OneDrivePicker: React.FC<OneDrivePickerProps> = ({
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      ...
      if (tab === 'myDrive') {
        ...
        // Raiz de "Partilhado comigo"
        if (!currentFolderId) {
          const data = await listSharedWithMe(token);
          setItems(data);
        } else {
          // Dentro de uma pasta partilhada — precisamos do driveId
  ...
  // Ao mudar de tab, reset breadcrumbs
